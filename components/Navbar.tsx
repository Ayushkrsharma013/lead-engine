...
export default function Navbar({ source, setSource, mock, setMock, savedCount }: NavbarProps) {
  ...
  return (
    <nav className="fixed top-0 left-0 right-0 z-50 h-14 border-b border-white/[0.06] bg-[#080b10]/95 backdrop-blur-md flex items-center px-5 gap-5">
      {/* Logo */}
        ...
        {TABS.map(tab => (
          <button key={tab.id} onClick={() => setSource(tab.id)}
            className={cn(
              ...
              source === tab.id ? "shadow-sm" : "text-slate-500 hover:text-slate-300"
            )}
      ...
      {/* Saved leads count */}
      {savedCount > 0 && (
        <div className="flex items-center gap-1.5 text-xs text-slate-400 bg-white/[0.04] border border-white/[0.06] rounded-lg px-3 py-1.5">
          <Database size={12} className="text-indigo-400" />
          <span><span className="font-semibold text-slate-200">{savedCount}</span> saved leads</span>
        </div>
      )}
