...
export default function ImportModal({ open, onClose, onImported }: ImportModalProps) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm">
      <div className="w-[640px] max-w-[96vw] max-h-[85vh] bg-surface border border-border rounded-2xl shadow-2xl flex flex-col overflow-hidden animate-fade-up">
        {/* Header */}
        <div className="flex items-center gap-3 px-6 py-4 border-b border-border shrink-0">
          <div className="w-8 h-8 rounded-lg flex items-center justify-center bg-accent-purple/15 border border-accent-purple/25">
            <CloudDownload size={16} style={{ color: "#7c3aed" }} />
          </div>
          <div className="flex-1">
            <p className="text-sm font-semibold text-text">Import Leads from Apify</p>
        ...
        <div className="flex-1 overflow-y-auto px-6 py-4 space-y-4">
          ...
          {phase === "loading" && (
            <div className="flex items-center justify-center py-16">
              ...
              <span className="ml-3 text-sm text-muted">Fetching Apify runs…</span>
            </div>
          )}
          ...
          {phase === "error" && (
            <div className="flex flex-col items-center justify-center py-10 gap-3">
              ...
              <button
                ...
                className="flex items-center gap-2 px-4 py-2 rounded-lg text-xs text-accent-blue bg-white/[0.04] border border-border hover:bg-white/[0.08] transition-all"
              >
            ...
            <>
              ...
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-2">
                  <button onClick={selectAll} className="text-[11px] text-accent-blue hover:text-accent-blue/80 transition-colors">Select all</button>
                  <span className="text-muted/50">•</span>
                  <button onClick={deselectAll} className="text-[11px] text-muted hover:text-text transition-colors">Deselect all</button>
                </div>
                <span className="text-xs text-muted">
                  <strong className="text-text">{selected.size}</strong> of {runs.length} runs •{" "}
                  <strong className="text-text">{totalSelectedLeads.toLocaleString()}</strong> leads
                </span>
              </div>
                  ...
                  return (
                    ...
                    >
                      ...
                      <div className="flex-1 min-w-0">
                        <p className="text-[12px] font-medium text-text truncate font-mono">
                          {run.runId.slice(0, 16)}…
                        </p>
                        <p className="text-[10px] text-muted">{formatDate(run.finishedAt)}</p>
                      </div>
                      <span className={cn(
              ...
              <div className="flex items-center gap-3 pt-2 border-t border-border">
                <label className="text-[11px] font-semibold text-text shrink-0">Leads per run:</label>
                <input
                  type="range"
                  min={10}
                  max={500}
                ...
                />
                <span className="text-[11px] font-mono text-text w-10 text-right">{limit}</span>
              </div>
            </>
          )}
          ...
          {phase === "done" && result && (
            <div className="flex flex-col items-center justify-center py-8 gap-3">
              ...
              <p className="text-sm font-semibold text-text">Import Complete</p>
              <p className="text-xs text-muted text-center">{message}</p>
              <div className="flex gap-6 mt-2">
                <div className="text-center">
                  <p className="text-lg font-bold text-accent-green">{result.added}</p>
                  <p className="text-[10px] text-muted">New</p>
                </div>
                <div className="text-center">
                  <p className="text-lg font-bold text-accent-blue">{result.updated}</p>
                  <p className="text-[10px] text-muted">Updated</p>
                </div>
                <div className="text-center">
                  <p className="text-lg font-bold text-text">{result.total}</p>
          ...
          {phase === "importing" && (
            <div className="flex items-center justify-center py-6 gap-3">
              ...
              <span className="text-sm text-muted">Importing {selected.size} run{selected.size > 1 ? "s" : ""}…</span>
            </div>
          )}
        </div>
