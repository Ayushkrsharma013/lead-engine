  ...
  return (
    <div className="flex flex-col items-center">
      ...
      <div className="absolute text-center" style={{ marginTop: -100 }}>
        <div className="text-[48px] font-bold text-text tabular-nums">{score}</div>
        <div className="text-xs text-muted">ICP Score</div>
      </div>
    </div>
  );
}
    ...
    <>
      ...
      <div className="flex-1 overflow-y-auto">
        ...
        <div className="flex items-center gap-3 px-6 py-3 border-b border-border bg-surface2">
          ...
          <input
            ...
            className="flex-1 h-8 rounded-md bg-white/5 border border-border px-3 text-xs text-text placeholder:text-muted focus:outline-none focus:border-accent-orange/40 font-mono"
          />
          <button
            onClick={handleConnect}
            disabled={!keyInput.trim()}
            className="flex items-center gap-1.5 h-8 px-4 rounded-md bg-accent-orange/20 text-accent-orange text-xs font-medium hover:bg-accent-orange/30 disabled:opacity-40 transition-colors"
          >
            {connected ? <><Check size={12} /> Connected</> : "Connect"}
          </button>
          {connected && <span className="w-2 h-2 rounded-full bg-accent-green shrink-0" />}
        </div>

        <div className="flex h-[calc(100vh-160px)]">
          {/* LEFT — Input (45%) */}
          <div className="w-[45%] min-w-[340px] border-r border-border p-5 space-y-5 overflow-y-auto">
            {/* Lead Info */}
            <div>
              <label className="text-[11px] font-semibold text-muted uppercase tracking-wider">Lead Information</label>
              <textarea
                value={leadText}
                onChange={e => setLeadText(e.target.value)}
                placeholder="Paste LinkedIn bio, job title, company info, or any raw text…"
                rows={10}
                className="w-full rounded-md bg-white/5 border border-border px-3 py-2 text-sm text-text placeholder:text-muted/70 focus:outline-none focus:border-accent-orange/40 resize-y mt-1.5"
              />
            </div>

            {/* ICP Criteria */}
            <div>
              <label className="text-[11px] font-semibold text-muted uppercase tracking-wider">ICP Criteria</label>
              <div className="space-y-2 mt-1.5">
                {criteria.map((c, i) => (
                  <label key={i} className="flex items-center gap-2.5 cursor-pointer group">
                    ...
                    />
                    <span className="text-xs text-text flex-1">{c.label}</span>
                    <span className="text-[10px] font-bold text-muted bg-white/5 px-1.5 py-0.5 rounded tabular-nums">{c.points} pts</span>
                  </label>
                ))}
              </div>
              <p className="text-[10px] text-muted mt-2">Max possible: {maxPoints} pts (with selected criteria)</p>
            </div>
            ...
            <button
              ...
              className="w-full flex items-center justify-center gap-2 h-11 rounded-lg bg-accent-orange text-white text-sm font-semibold hover:bg-accent-orange/90 disabled:opacity-40 disabled:cursor-not-allowed transition-colors"
            >
              {scoring ? (
          ...
          <div className="flex-1 p-5 space-y-5 overflow-y-auto flex flex-col items-center">
            {result ? (
              <>
                ...
                {/* Reasoning */}
                <div className="w-full max-w-md bg-surface border border-border rounded-lg p-4">
                  <h4 className="text-sm font-semibold text-text mb-2">Why this score</h4>
                ...
                <div className="w-full max-w-md">
                  <span className="text-[10px] text-muted uppercase tracking-wider">Recommended Action</span>
                ...
                {result.risk_factors.length > 0 && (
                  <div className="w-full max-w-md">
                    <details className="group">
                      <summary className="flex items-center gap-1.5 text-xs font-semibold text-muted cursor-pointer hover:text-text transition-colors">
                ...
                <button
                  ...
                  className="flex items-center gap-2 h-10 px-4 rounded-lg bg-accent-green/10 text-accent-green border border-accent-green/25 text-sm font-medium hover:bg-accent-green/20 transition-colors"
                >
      ...
      {showAddModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm" onClick={() => setShowAddModal(false)}>
          <div className="w-[400px] max-w-[95vw] bg-surface border border-border rounded-xl shadow-2xl p-6 space-y-4 animate-fade-up" onClick={e => e.stopPropagation()}>
            <h3 className="text-sm font-semibold text-text">Add to Pipeline</h3>
            <div className="space-y-3">
              <input
                type="text" placeholder="Name *"
                value={addName} onChange={e => setAddName(e.target.value)}
                className="w-full h-9 rounded-md bg-white/5 border border-border px-3 text-sm text-text placeholder:text-muted focus:outline-none focus:border-accent-green/40"
              />
              <input
              ...
              <div className="flex items-center gap-2">
                ...
                <span className="text-sm font-bold text-text">{result?.score ?? 0}</span>
              </div>
            </div>
            <div className="flex gap-2 pt-2">
              <button onClick={handleAddToPipeline} disabled={!addName.trim()} className="flex-1 h-9 rounded-md bg-accent-green/20 text-accent-green text-sm font-medium hover:bg-accent-green/30 disabled:opacity-40 transition-colors">
                Save Lead
              </button>
              <button onClick={() => setShowAddModal(false)} className="flex-1 h-9 rounded-md bg-white/5 text-muted text-sm hover:bg-white/[0.08] transition-colors">
                Cancel
              </button>
            </div>
          </div>
