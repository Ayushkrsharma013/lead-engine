      ...
      <div className="max-w-3xl space-y-4 animate-fade-in">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
          {CONNECTORS.map(c => {
            ...
            return (
              <div key={c.type} className="rounded-xl p-5" style={{ background: "var(--surface)", border: isConfigured ? "2px solid rgba(34,197,94,0.2)" : "1px solid var(--line)" }}>
                <div className="flex items-center justify-between mb-3">
                  <div className="flex items-center gap-2.5">
                    <c.icon size={18} style={{ color: isConfigured ? "#22c55e" : "var(--ink-2)" }} />
                ...
                <div className="flex gap-2">
                  <button onClick={() => openModal(c.type)}
                  ...
                  {isConfigured && (
                    <button onClick={() => handleDisconnect(c.type)}
        ...
        {modalType && (
          <div style={{ position: "fixed", inset: 0, zIndex: 100, display: "flex", alignItems: "center", justifyContent: "center", background: "rgba(0,0,0,0.6)", backdropFilter: "blur(4px)" }}
            ...
            <div className="rounded-2xl p-6 w-full max-w-md" style={{ background: "var(--surface-elev)", border: "1px solid var(--line)", boxShadow: "var(--shadow-lg)" }}>
              <div className="flex items-center justify-between mb-4">
                <h2 className="text-[15px] font-bold" style={{ color: "var(--ink)" }}>
                  Configure {CONNECTORS.find(c => c.type === modalType)?.label}
                </h2>
                <button onClick={() => setModalType(null)} style={{ background: "none", border: "none", cursor: "pointer", color: "var(--ink-3)" }}><X size={16} /></button>
              </div>
