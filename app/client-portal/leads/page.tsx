...
export default function ClientLeadsPage() {
  ...
  const limit=25;const effMin=view==="hot"?80:scoreMin;

  useEffect(()=>{ (async()=>{ const meRes=await fetch("/prospecting-os/api/client-portal/me"); if(meRes.ok) setProfile((await meRes.json()).profile); const hotRes=await fetch("/prospecting-os/api/client-portal/leads?limit=1&score_min=80"); if(hotRes.ok) setHotCount((await hotRes.json()).count||0); })(); },[]);
