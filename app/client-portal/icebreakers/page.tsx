...
export default function ClientIcebreakersPage() {
  ...
  if (loading) {
    return (
      <div className="p-6 md:p-10 max-w-5xl mx-auto space-y-4">
        {[1,2,3].map(i => (
          <div key={i} className="rounded-xl p-5 animate-pulse" style={{ background: "var(--surface)", border: "1px solid var(--line)" }}>
            <div className="h-4 w-48 rounded mb-2" style={{ background: "rgba(255,255,255,0.04)" }} />
  ...
  return (
    <PlanGate module="icebreakers" plan={profile?.plan as PlanKey || null} role={profile?.role} requiredPlan="growth">
      ...
      <div className="p-6 md:p-10 max-w-5xl mx-auto" style={{ position: "relative", zIndex: 1 }}>
        <motion.div
          ...
          className="mb-6"
        >
          <h1 className="text-[20px] font-bold tracking-tight" style={{ color: "var(--ink)" }}>Icebreakers</h1>
          <p className="text-[13px] mt-1" style={{ color: "var(--ink-3)" }}>
            AI-generated icebreakers for your top leads (score 60+)
          </p>
        </motion.div>
        ...
        {leads.length === 0 ? (
          ...
          >
            ...
            <p className="text-[12px] mt-1" style={{ color: "var(--ink-4)" }}>
              Leads with scores above 60 will appear here with AI-generated icebreaker messages.
            </p>
          </motion.div>
        ) : (
          <div className="space-y-3">
            {leads.map((lead, i) => (
              ...
              >
                ...
                >
                  <div>
                    <p className="text-[13px] font-semibold" style={{ color: "var(--ink)" }}>{lead.name || "—"}</p>
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <motion.div animate={{ rotate: expandedId === lead.id ? 180 : 0 }} transition={{ duration: 0.2 }}>
                          ...
                          >
                            <div className="flex items-center gap-2 mb-1.5">
                              <span className="text-[9px] font-bold uppercase px-1.5 py-0.5 rounded"
                                style={{ background: "rgba(232,74,10,0.08)", color: "#E84A0A" }}>
                                {ib.tone || "friendly"}
                              </span>
                              {ib.subject && (
                              ...
                              <button
                                ...
                                className="ml-auto p-1.5 rounded-lg transition-all opacity-0 group-hover:opacity-100"
                                style={{ color: copiedId === ib.id ? "var(--positive)" : "var(--ink-4)", background: "transparent", border: "none", cursor: "pointer" }}
